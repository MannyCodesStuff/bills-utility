...
export function BillManager2() {
  ...
  const handleUploadCreditMemo = async (data: CreditMemoSchemaType) => {
    ...
    try {
      ...
      let newFileName = `${data.documentType}_${formattedDate}.pdf`
      if (creditMemoNumber) {
        newFileName = `${data.documentType}_${creditMemoNumber}_${formattedDate}.pdf`
      }
      ...
      const uploadResult = await uploadFileToSharePoint(serverPort, {
        filePath: newFilePath,
        vendorId: null,
        invoiceNumber: data.creditMemoNumber || null,
