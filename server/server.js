...
async function startServer() {
  ...
  async function insertInvoiceToDatabase(
    ...
    try {
      ...
      const result = await pool
        ...
        .query(
          'INSERT INTO LOCDATAMART.dbo.SHAREPOINT_INVOICES (vendor_id, invoice_number, invoice_date, sharepoint_url, original_path, uploaded, store) VALUES (@vendor_id, @invoice_number, @invoice_date, @sharepoint_url, @original_path, 0, @store)'
        )
  ...
  app.post('/upload-pdf', async (req, res) => {
    ...
    if (documentType === 'invoice') {
      if (!filePath || !vendorId || !invoiceNumber || !invoiceDate || !store) {
